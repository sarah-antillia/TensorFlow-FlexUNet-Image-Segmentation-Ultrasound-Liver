...
class ImageMaskDatasetGenerator:
  ...
  def drawFilledPolygon(self, mask, points, color):
    (r, g, b) = color
    bgr_color = (b, g, r)
  ...
  def generate(self, image_file, json_file, output_images_dir, output_masks_dir, category):
    print(image_file)
    self.index += 1
    filename =str(self.index) + ".png"
    print(json_file)
    color = self.RGB_COLORS[category]
    print("Color", color)
    ...
    with open(json_file, 'r') as f:
      ...
      self.drawFilledPolygon(mask, points, color)
      ...
      if self.augmentation:
        self.augment(mask, filename, output_masks_dir, border=(0, 0, 0),     mask=True)
...
if __name__ == "__main__":
  try:
    root_dir = "./archive/"
    output_dir     = "./Augmented-Ultrasound-Liver/"
    ...
    for subdir in subdirs:
      ...
      if subdir == "Normal":
        continue
      subdir_fullpath = os.path.join(root_dir, subdir)
      images_dir      = os.path.join(subdir_fullpath, "image")
      labels_dir      = os.path.join(subdir_fullpath, "segmentation/mass")
      image_files     = sorted(glob.glob(images_dir + "/*.jpg"))
      json_files      = sorted(glob.glob(labels_dir + "/*.json"))

      num_images  = len(image_files)
      num_jsons   = len(json_files)
      num = min(num_images, num_jsons)
      for i in range(num):
        print(image_files[i])
        print(json_files[i])

        generator.generate(image_files[i], json_files[i], output_images_dir, output_masks_dir, subdir)
